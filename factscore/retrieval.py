...
class Retrieval:
    ...
    async def get_texts_for_queries(self, queries: Union[str, list[str]], k: int) -> dict[str, list[dict]]:
        '''
    ...
    def get_bm25_passages(self, topic: str, fact: str, chunks: list[dict], n: int) -> list[dict]:
        ...
        bm25 = BM25Okapi([text["text"].replace("<s>", "").replace("</s>", "").split() for text in chunks])
